...
class DepartmentManager(PetabytzModel):
    ...
    objects = PetabytzCompanyManager("manager__employee_work_info__company_id")
...
class TicketType(PetabytzModel):
...
class ClaimRequest(PetabytzModel):
...
class Attachment(PetabytzModel):
...
class FAQCategory(PetabytzModel):
