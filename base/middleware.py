...
class CompanyMiddleware:
    ...
    def _set_company_session(self, request, company_id):
        ...
        try:
            ...
            if request.user.is_superuser:
                # Superusers can login without an employee profile
                pass
            elif request.user.is_staff:
                # Auto-provision Employee profile for Staff (Self-Healing)
                try:
                    user_employee = Employee.objects.filter(email=request.user.email).first()
                    if not user_employee:
                        user_employee = Employee.objects.create(
                        ...
                        # Link if orphaned
                        user_employee.employee_user_id = request.user
                        user_employee.save()
                    
                    # Ensure Work Info & Company Assignment
                    if not hasattr(user_employee, 'employee_work_info'):
                        # Infer company from request (multi-tenant) or username
                        company = getattr(request, 'company', None)
                        if not company and "." in request.user.username:
                            c_name = request.user.username.split(".")[-1]
                            company = Company.objects.filter(company__iexact=c_name).first()
                    ...
                    # If auto-provisioning fails, enforce logout
                    logout(request)
                    messages.error(
                        request,
        ...
        user_company_id = getattr(
            getattr(user_employee, "employee_work_info", None), "company_id", None
        )
    ...
    def _add_company_filter(self, model, company_id):
        ...
        is_horilla_manager = isinstance(model.objects, PetabytzCompanyManager)
