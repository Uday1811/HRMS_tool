...
from horilla_audit.models import PetabytzAuditInfo, PetabytzAuditLog
...
class Company(PetabytzModel):
    ...
    email_domain = models.CharField(
        ...
        help_text=_("Email domain for auto-detecting company (e.g., petabytz.com)")
    )
    timezone = models.CharField(
        max_length=50,
        default="Asia/Kolkata",
        verbose_name=_("Timezone"),
        help_text=_("Default timezone for the company (e.g., Asia/Kolkata, America/New_York)")
    )
    country_code = models.CharField(
        ...
        help_text=_("ISO country code for holiday calendar (e.g., IN, US, BD)")
    )
    is_multi_location = models.BooleanField(
        default=False,
        verbose_name=_("Multi-Location Company"),
        help_text=_("Enable if company has multiple locations with different timezones/holidays")
    )
    ...
    def get_company_by_email(email):
        ...
        try:
            return Company.objects.get(email_domain__iexact=domain)
        except Company.DoesNotExist:
            return None


class Department(PetabytzModel):
    """
    Department model
    """
...
class JobPosition(PetabytzModel):
    """
    JobPosition model
    """
    ...
    objects = PetabytzCompanyManager("department_id__company_id")
...
class JobRole(PetabytzModel):
    ...
    objects = PetabytzCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(PetabytzModel):
    """
...
class RotatingWorkTypeAssign(PetabytzModel):
    """
...
class EmployeeType(PetabytzModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(PetabytzModel):
    """
    EmployeeShift model
    """
...
class EmployeeShiftSchedule(PetabytzModel):
    """
...
class RotatingShift(PetabytzModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(PetabytzModel):
    """
...
class WorkTypeRequest(PetabytzModel):
    """
...
class WorkTypeRequestComment(PetabytzModel):
    """
...
class ShiftRequest(PetabytzModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(PetabytzModel):
    """
...
class Tags(PetabytzModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = PetabytzCompanyManager(related_company_field="company_id")
...
class PetabytzMailTemplate(PetabytzModel):
...
class DynamicEmailConfiguration(PetabytzModel):
    """
...
class MultipleApprovalCondition(PetabytzModel):
...
class Announcement(PetabytzModel):
    """
...
class AnnouncementComment(PetabytzModel):
    """
...
class DashboardEmployeeCharts(PetabytzModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(PetabytzModel):
...
class Holidays(PetabytzModel):
    ...
    location = models.CharField(
        ...
        help_text=_("Specific location for this holiday (e.g., India, Dhaka). Leave blank for all locations.")
    )
    objects = PetabytzCompanyManager(related_company_field="company_id")
    ...
    @staticmethod
    def get_holidays_for_employee(employee, start_date=None, end_date=None):
        """
        Get holidays applicable to an employee based on their company and location.
        
        Args:
            employee: Employee instance
            start_date: Start date for holiday range (optional)
            end_date: End date for holiday range (optional)
            
        Returns:
            QuerySet of applicable holidays
        """
        company = employee.get_company()
        if not company:
            return Holidays.objects.none()
        
        # Get employee location from work info
        location = getattr(getattr(employee, 'employee_work_info', None), 'location', None)
        
        # Build query
        query = models.Q(company_id=company)
        
        # If company is multi-location and employee has a location, filter by location
        if company.is_multi_location and location:
            query &= (models.Q(location__iexact=location) | models.Q(location__isnull=True))
        
        holidays = Holidays.objects.filter(query)
        
        # Apply date range if provided
        if start_date:
            holidays = holidays.filter(end_date__gte=start_date)
        if end_date:
            holidays = holidays.filter(start_date__lte=end_date)
            
        return holidays


class CompanyLeaves(PetabytzModel):
...
class PenaltyAccounts(PetabytzModel):
    """
