...
def white_labelling_company(request):
    ...
    if white_labelling:
        ...
        return {
            "white_label_company_name": company.company if company else "Petabytz",
            "white_label_company": company,
        }
    else:
        return {
            "white_label_company_name": "Petabytz",
...
def company_context(request):
    """
    Add company information to template context for all requests.
    This makes request.company available in all templates for multi-tenant support.
    """
    context = {}
    
    # Get company from request (set by MultiTenantMiddleware)
    if hasattr(request, 'company') and request.company:
        context['current_company'] = request.company
        context['current_company_name'] = request.company.company
        context['current_company_timezone'] = request.company.timezone
    # Fallback to session
        ...
        try:
            ...
            context['current_company_name'] = company.company
            context['current_company_timezone'] = company.timezone
