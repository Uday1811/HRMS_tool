...
from horilla.filters import PetabytzPaginator
from horilla.group_by import group_by_queryset
from horilla.horilla_settings import PETABYTZ_DATE_FORMATS
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = PetabytzPaginator(qryset, get_pagination())
...
def employee_view(request):
    ...
    # Get hierarchy data for the logged-in employee
    employee = request.user.employee_get
    reporting_manager = employee.get_reporting_manager()
    
    # Get peers (employees with same reporting manager and same job position)
    peers = []
    if reporting_manager:
        job_position = employee.get_job_position()
        peers = Employee.objects.filter(
            employee_work_info__reporting_manager_id=reporting_manager,
            employee_work_info__job_position_id=job_position,
            is_active=True
        ).exclude(id=employee.id)[:10]  # Limit to 10 peers
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        PETABYTZ_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def employee_dashboard(request):
    """
    Employee dashboard view with comprehensive overview
    """
    employee = request.user.employee_get
    ...
    try:
        ...
        # 2. Group by date for "Logs" table
        # Structure: Date | Visual (first in - last out) | Effective (sum duration) | Gross | Arrival | Log
        grouped_logs = []
        # groupby expects sorted data by key
        logs_list = list(logs)
        logs_list.sort(key=lambda x: x.timestamp.date(), reverse=True)
        
        for date_key, group in groupby(logs_list, key=lambda x: x.timestamp.date()):
            daily_logs = list(group)
            daily_logs.sort(key=lambda x: x.timestamp) # Sort ASC for processing
            
            first_in = next((x for x in daily_logs if x.action == 'in'), None)
            last_out = next((x for x in reversed(daily_logs) if x.action == 'out'), None)
            
            effective_seconds = 0
            # Rough Calc: Last Out - First In (Ignores breaks for simplicity now)
            if first_in:
                end_time = last_out.timestamp if last_out else timezone.now()
                if end_time > first_in.timestamp:
                    effective_seconds = (end_time - first_in.timestamp).total_seconds()
            
            eff_hrs = int(effective_seconds // 3600)
            eff_mins = int((effective_seconds % 3600) // 60)

            # Arrival status
            arrival_status = "On Time"
            if first_in:
                 # Mock shift start 10:00 AM
                 shift_start = first_in.timestamp.replace(hour=10, minute=0, second=0, microsecond=0)
                 if first_in.timestamp > shift_start + timedelta(minutes=15):
        ...
        for log_data in grouped_logs:
             # Parse effective hours back to seconds for avg calc
             eff_str = log_data['effective_hours'].split(' ')[0] # "H:M"
             h, m = map(int, eff_str.split(':'))
             total_effective_seconds += h * 3600 + m * 60
             
             if log_data['first_in']:
                 total_days_worked += 1
                 if "Late" not in log_data['arrival']:
                     total_on_time += 1
        
        avg_seconds = total_effective_seconds / total_days_worked if total_days_worked > 0 else 0
        avg_h = int(avg_seconds // 3600)
        avg_m = int((avg_seconds % 3600) // 60)
        
        on_time_percentage = int((total_on_time / total_days_worked) * 100) if total_days_worked > 0 else 0
...
def apply_leave_api(request):
    ...
    try:
        ...
        # Extract form data
        leave_type = request.POST.get('leave_type')  # 'paid' or 'sick'
        ...
        if not all([leave_type, duration, start_date, end_date, reason]):
            return JsonResponse({
                'success': False, 
        ...
        from employee.models import EmployeeLeaveRequest, EmployeeLeaveBalance
        ...
        if attachments and len(attachments) > 0:
            leave_request.attachment = attachments[0]  # Take first attachment
            leave_request.save()
        
        # Send notification to manager (if available)
        manager = leave_request.get_manager()
        if manager:
        ...
        return JsonResponse({
            'success': True, 
            'message': 'Leave application submitted successfully and is pending manager approval',
...
@login_required
def cancel_leave_api(request, request_id):
    """
    API endpoint for cancelling leave request
    """
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        from employee.models import EmployeeLeaveRequest
        
        # Get leave request
        try:
            leave_request = EmployeeLeaveRequest.objects.get(
                id=request_id, 
                employee=employee
            )
        ...
        # Cancel the request (this will restore balance if approved)
        leave_request.cancel_request()
...
def approve_leave_api(request, request_id):
    """
    API endpoint for managers to approve leave requests
    """
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        if leave_request.manager != manager_employee:
            return JsonResponse({
                'success': False, 
        ...
        # Approve the request
        leave_request.approve_request(manager_employee, comments)
...
@login_required
def reject_leave_api(request, request_id):
    """
    API endpoint for managers to reject leave requests
    """
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        # Reject the request
        leave_request.reject_request(manager_employee, comments)
...
@login_required
def get_leave_balance_api(request, leave_type):
    """
    API endpoint to get employee leave balance
    """
    try:
        employee = request.user.employee_get
        current_year = date.today().year
        
        from employee.models import EmployeeLeaveBalance
        ...
        try:
            ...
            return JsonResponse({
                ...
                'balance': {
                    ...
                    'leave_type': balance.get_leave_type_display(),
                    'year': balance.year
                }
            })
...
def attendance_dashboard(request):
    """
    Comprehensive attendance dashboard for employees
    """
    employee = request.user.employee_get
...
def finance_dashboard(request):
    """
    Finance dashboard for employees - payslips, salary info
    """
    employee = request.user.employee_get
    ...
    return render(request, "employee/finance_dashboard.html", context)


@login_required
def handbook_dashboard(request):
    """
    Employee handbook dashboard
    """
    employee = request.user.employee_get
...
def policy_dashboard(request):
    """
    Policy dashboard for employees
    """
    employee = request.user.employee_get
...
def leaves_dashboard(request):
    """
    Render the Leave Management Dashboard with Keka-style metrics and UI.
    """
    from employee.models import EmployeeLeaveBalance, EmployeeLeaveRequest
    from django.contrib import messages
    
    try:
        employee = request.user.employee_get
    except Exception:
        messages.error(request, "You must be an employee to view this dashboard.")
        return redirect('/')

    today = timezone.now().date()
    current_year = today.year
    
    # Ensures standard balances exist for the employee
    leave_types = ['cl', 'sl', 'el', 'wfh', 'comp_off', 'lop']
    ...
    # Get Balances
    balances = EmployeeLeaveBalance.objects.filter(employee=employee, year=current_year)
    balance_dict = {b.leave_type: b for b in balances}
    
    # Defaults if not present
    def get_bal(l_type):
        b = balance_dict.get(l_type)
        return {'available': b.available_days if b else 0, 'total': b.total_accrued if b else 0}

    context = {
        'employee': employee,
        'cl_balance': get_bal('cl'),
        'sl_balance': get_bal('sl'),
        'el_balance': get_bal('el'),
        'wfh_balance': get_bal('wfh'),
        'pending_requests': EmployeeLeaveRequest.objects.filter(employee=employee, status='pending').count(),
        'recent_requests': EmployeeLeaveRequest.objects.filter(employee=employee).order_by('-created_at')[:5],
        'current_year': current_year,
    }
    
    return render(request, "employee/leave/dashboard.html", context)


@login_required
def apply_leave_api(request):
    """
    API view to handle leave application.
    GET: Returns the form HTML (for modal).
    POST: Processes the leave request.
    """
    from employee.models import EmployeeLeaveRequest, EmployeeLeaveBalance, CompOffRequest
    from django.core.exceptions import ValidationError
    
    if request.method == 'GET':
        return render(request, "employee/leave/apply_leave_modal.html")
    ...
    if request.method == 'POST':
        try:
            ...
            # Create request object (valiation happens in save/clean, but we can try/catch)
            ...
            leave_request.full_clean() # Triggers logic in models.py
            leave_request.save()
            ...
            return JsonResponse({'status': 'error', 'message': str(e.message_dict if hasattr(e, 'message_dict') else e)})
        except Exception as e:
...
def get_leave_balance_api(request, leave_type):
    """
    API to fetch available balance for a specific leave type.
    """
    from employee.models import EmployeeLeaveBalance
    ...
    try:
        ...
        return JsonResponse({
            ...
            'total': float(balance.total_accrued)
        })
    except EmployeeLeaveBalance.DoesNotExist:
        return JsonResponse({
            'status': 'success', 
...
def cancel_leave_api(request, request_id):
    """
    API to cancel a leave request.
    """
    from employee.models import EmployeeLeaveRequest
    from django.utils.translation import gettext as _
    try:
        req = EmployeeLeaveRequest.objects.get(id=request_id, employee=request.user.employee_get)
        if req.status == 'cancelled':
             return HttpResponse(_("Already cancelled"))
        
        req.cancel_request()
        messages.success(request, _("Leave request cancelled"))
        return HttpResponse('<script>window.location.reload();</script>')
    except EmployeeLeaveRequest.DoesNotExist:
         return HttpResponse(_("Request not found"), status=404)
    except Exception as e:
...
def approve_leave_api(request, request_id):
    """
    API to approve a leave request (Manager only).
    """
    from employee.models import EmployeeLeaveRequest
    from django.utils.translation import gettext as _
    try:
        req = EmployeeLeaveRequest.objects.get(id=request_id)
        # Permission check: simplistic
        if req.manager != request.user.employee_get and not request.user.has_perm('leave.approve_leave'):
             # If user is admin/superuser they might have permission
             pass
            
        req.approve_request(manager=request.user.employee_get)
...
def reject_leave_api(request, request_id):
    ...
    try:
        ...
        req.reject_request(manager=request.user.employee_get)
...
def manager_leave_requests(request):
    """
    View for managers to see pending leave requests.
    """
    from employee.models import EmployeeLeaveRequest
    
    employee = request.user.employee_get
    # Requests where this user is the manager
    requests = EmployeeLeaveRequest.objects.filter(manager=employee).order_by('-created_at')
    
    context = {
        'requests': requests,
        'employee': employee
    }
    return render(request, "employee/leave/manager_approvals.html", context)


@login_required
def holiday_calendar(request):
    """
    View to show Holiday Calendar.
    """
    from leave.models import Holiday
    today = timezone.now().date()
    # Show upcoming holidays
    holidays = Holiday.objects.filter(start_date__gte=today).order_by('start_date')
