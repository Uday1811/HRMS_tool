...
def clock_in_attendance_and_activity(
    ...
    today_long = None
    try:
        today_lat = float(_thread_locals.request.GET.get("latitude"))
        today_long = float(_thread_locals.request.GET.get("longitude"))
    except:
        pass
    ...
    new_activity = AttendanceActivity.objects.create(
        ...
        clock_in_latitude=today_lat,
        clock_in_longitude=today_long,
    )
...
def clock_in(request):
    ...
    if (
        ...
        if employee and work_info is not None:
            ...
            context = {
                "check_in": False, # User is checked in, so button should be Check Out (logic inverted in template check)
                                   # Actually, if user is CLOCKED IN, we want to show CLOCK OUT button.
                                   # My template logic: {% if check_in %} -> Show Clock In button.
                                   # So if user is clocked IN, check_in should be False.
            }
            # Logic verification:
            # If user successfully clocked IN, they are now IN. Next action is OUT.
            # So we pass check_in=False (next action is not check in).
            
            return render(request, "attendance/components/in_out_component.html", {"check_in": False})
        return HttpResponse(
            _(
    ...
    if attendance_activities.filter(clock_out__isnull=True).exists():
        ...
        try:
            ...
            if request:
                ...
                attendance_activity.clock_out_latitude = lat
                attendance_activity.clock_out_longitude = lon
        except:
             pass
...
def clock_out(request):
    ...
    if (
        ...
        if time_runner_enabled:
            ...
            return render(request, "attendance/components/in_out_component.html", {"check_in": True}) # User clocked OUT, so next is Check IN
    else:
        messages.error(request, _("Check in/Check out feature is not enabled."))
        return HttpResponse("<script>location.reload();</script>")

def render_clock_widget(request):
    """
    Renders the clock widget with the current status of the employee.
    """
    try:
        employee = request.user.employee_get
        today = date.today()
        # Find if the user is currently clocked in.
        # User is clocked IN if there is an activity today with NULL clock_out.
        ...
        # If user is clocked IN, check_in context should be False (show Clock OUT button)
        # If user is NOT clocked IN, check_in context should be True (show Clock IN button)
        check_in_status = not is_clocked_in
        
        return render(request, "attendance/components/in_out_component.html", {"check_in": check_in_status, "unique": "dashboard_load"})
