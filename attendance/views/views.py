...
from horilla.horilla_settings import DYNAMIC_URL_PATTERNS, PETABYTZ_DATE_FORMATS
...
def employee_dashboard_view(request):
    """Thin wrapper that redirects to the main employee dashboard.
    This satisfies the URL pattern ``employee-dashboard/`` required by the
    attendance app without altering the existing UI.
    """
    # The employee dashboard URL name is defined in the employee app.
    # If the name changes, adjust the reverse() argument accordingly.
    return redirect(reverse("employee-dashboard"))



# --------------------------------------------------------------------
# Clock-in / Clock-out API wrappers
# --------------------------------------------------------------------
import json
...
def clock_in_api(request):
    from attendance.views import clock_in_out as attendance_clock
    return attendance_clock.clock_in(request)

@login_required
def clock_out_api(request):
    from attendance.views import clock_in_out as attendance_clock
    return attendance_clock.clock_out(request)
...
def ajax_clock(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'status': 'success', 'message': f'Successfully {action}ed'})
...
def work_record_export(request):
    ...
    format_string = PETABYTZ_DATE_FORMATS.get(date_format)
