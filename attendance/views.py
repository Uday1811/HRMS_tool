...
def clock_in(request):
    ...
    if employee and work_info is not None:
        ...
        if start_time_sec > end_time_sec:
            # night shift
            # ------------------
            # Night shift in Petabytz consider a 24 hours from noon to next day noon,
...
def employee_dashboard_view(request):
    """
    Modern employee attendance dashboard with Keka-inspired design
    Features:
    - Web Clock-in/Clock-out with location tracking
    - Live stopwatch timer after login
    - Automatic 9-hour logout
    - Monthly stats and attendance logs
    - Missing login/logout detection
    """
    try:
        employee = request.user.employee_get
    except:
        messages.error(request, _("Please complete your employee profile first"))
        return redirect("/employee/employee-profile")
    ...
    is_clocked_in = last_activity is not None
    clock_in_time = None
    
    if is_clocked_in and last_activity:
        # Combine date and time for JavaScript
        clock_in_datetime = datetime.combine(
            last_activity.clock_in_date, 
            last_activity.clock_in
        )
        clock_in_time = clock_in_datetime.isoformat()
    ...
    for att in month_attendances:
        if att.at_work_second:
            total_seconds += att.at_work_second
    
    total_hours = format_time(total_seconds)
    
    # Calculate average hours
    avg_seconds = total_seconds // present_days if present_days > 0 else 0
    avg_hours = format_time(avg_seconds)
    
    # Get overtime
    overtime_obj = AttendanceOverTime.objects.filter(
        employee_id=employee,
        month=today.strftime("%B").lower(),
        year=current_year
    ).first()
    
    overtime = overtime_obj.overtime if overtime_obj else "0:00"
    
    # Missing logs (attendances without clock out)
    missing_logs = month_attendances.filter(
    ...
    working_days = current_day  # Simplified, should exclude weekends/holidays
    attendance_percentage = round((present_days / working_days * 100)) if working_days > 0 else 0
    
    # Recent attendances (last 10)
    recent_attendances = month_attendances[:10]
    
    # Get last activity for display
    last_activity_display = AttendanceActivity.objects.filter(
        employee_id=employee
    ).order_by('-id').first()
...
def clock_in_api(request):
    """
    API endpoint for clocking in with location tracking
    """
    import json
    ...
    try:
        ...
        if not work_info:
            return JsonResponse({
                'success': False,
                'message': _('Work information not found')
            })
        ...
        if existing_activity:
            return JsonResponse({
                'success': False,
                'message': _('You are already clocked in')
            })
        
        # Clock in logic (reuse existing clock_in function logic)
        shift = work_info.shift_id
        date_today = date.today()
        attendance_date = date_today
        day = date_today.strftime("%A").lower()
        ...
        return JsonResponse({
            'success': True,
            'message': _('Clocked in successfully'),
            'time': now
        })
...
def clock_out_api(request):
    """
    API endpoint for clocking out with location tracking
    """
    import json
    ...
    try:
        ...
        if not existing_activity:
            return JsonResponse({
                'success': False,
                'message': _('You are not clocked in')
            })
        ...
        return JsonResponse({
            'success': True,
            'message': _('Clocked out successfully'),
            'time': now
        })
